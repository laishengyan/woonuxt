...
<template>
  <div>
    <div class="cursor-pointer flex font-semibold mt-8 leading-none justify-between items-center" @click="isOpen = !isOpen">
      <span>{{ $t("messages.product.saleProductsOnly") }}</span>
    ...
    <div v-if="isOpen" class="mt-3 mr-1 max-h-[240px] grid gap-1 overflow-auto custom-scrollbar">
      <div class="flex gap-2 items-center">
        <label for="sale-true" class="cursor-pointer m-0 text-sm sr-only" aria-label="Only show products on sale"> {{ $t("messages.product.onlyProductsSale") }}</label>
        <input id="sale-true" v-model="selectedTerms" type="checkbox" :value="true" @click="checkboxClicked" />
      </div>
    </div>
  </div>
</template>
