...
const twitterUrl = computed(() => {
  if (props.productReview) {
    return `https://twitter.com/intent/tweet?text=${props.productReview.content}&url=${window.location.href}`
  } else {
    return `https://twitter.com/intent/tweet?text=${props.product.name}&url=${window.location.href}`
  }
});
...
const pinterestUrl = computed(
  () => {
    if (props.productReview) {
      return `https://pinterest.com/pin/create/button/?url=${window.location.href}&media=${props.productReview.images && props.productReview.images[0] && props.productReview.images[0].sourceUrl}&description=${props.productReview.content}`
    } else {
      return `https://pinterest.com/pin/create/button/?url=${window.location.href}&media=${props.product.image.sourceUrl}&description=${props.product.name}`
    }
  },
);
